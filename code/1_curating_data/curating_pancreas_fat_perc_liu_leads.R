...
clean_data = raw_data[which(raw_data$trait == "Fat" & raw_data$organ == "Pancreas"),] #10
...
pancreas_fat_perc = fread("output/1_curated_data/pancreas_fat_perc_liu_full_sumstats.txt") #full sumstats here do not have INDELS
...
for(index in seq(1, length(indel$chr_pos))){
  ...
  if(length(proxies) > 1){
    ...
    pancreas_fat_perc_match <- pancreas_fat_perc[which(pancreas_fat_perc$chr_pos%in%proxies_r2_08$Coord),]
    
    #Great, let's find the one with the highest r2 in the original one:
    
    proxies_match <- proxies_r2_08[which(proxies_r2_08$Coord%in%pancreas_fat_perc_match$chr_pos),]
    
    proxies_best = proxies_match[which.max(as.numeric(proxies_match$R2)),]
    
    #And retrieve it:
    
    pancreas_fat_perc_best = pancreas_fat_perc_match[which(pancreas_fat_perc_match$chr_pos%in%proxies_best$Coord),]
    
    #Just in case we have several that have R2:
    
    pancreas_fat_perc_best <- pancreas_fat_perc_best[which.min(as.numeric(pancreas_fat_perc_best$p_value)),]
    ...
    pancreas_fat_perc_best <- pancreas_fat_perc_best[1,]
    
    pancreas_fat_perc_best$lead=snp #we will remove later, just to trace how many we ended up recovering
    
    #Awesome -  let's go and save this:
    
    if(!(exists("recovered_snps"))){
      
      recovered_snps=pancreas_fat_perc_best
      
    } else {
      
      recovered_snps <- rbind(recovered_snps, pancreas_fat_perc_best)
...
non_indels$sample_size = pancreas_fat_perc$sample_size[1] #we can do this since all SNPs have the same sample size for this case
...
lead_df <- pancreas_fat_perc[which(pancreas_fat_perc$chr_pos%in%recovered_snps$chr_pos | pancreas_fat_perc$chr_pos%in%non_indels$chr_pos),] #9 - we lose one
...
fwrite(lead_df, "output/1_curated_data/pancreas_fat_perc_agrawal_leads.txt")
