...
#This code reads the supplementary table 3 and retrieves that lead ASATadjBMI from Agrawal et al. Data is available here: https://static-content.springer.com/esm/art%3A10.1038%2Fs41467-022-30931-2/MediaObjects/41467_2022_30931_MOESM4_ESM.xlsx
...
###################################################
#Let's filter the data for asatadjbmi - here VATadj#
###################################################

clean_data <- raw_data_[raw_data_$trait =="ASATadj",]

#We have several indels that we should take care of with LDLink and our newly curated data!

asatadjbmi = fread("output/1_curated_data/asatadjbmi_agrawal_full_sumstats.txt") #full sumstats here do not have INDELS
...
for(index in seq(1, length(indel$chr_pos))){
  ...
  if(length(proxies) > 1){
    ...
    asatadjbmi_match <- asatadjbmi[which(asatadjbmi$chr_pos%in%proxies_r2_08$Coord),]
    
    #Great, let's find the one with the highest r2 in the original one:
    
    proxies_match <- proxies_r2_08[which(proxies_r2_08$Coord%in%asatadjbmi_match$chr_pos),]
    
    proxies_best = proxies_match[which.max(as.numeric(proxies_match$R2)),]
    
    #And retrieve it:
    
    asatadjbmi_best = asatadjbmi_match[which(asatadjbmi_match$chr_pos%in%proxies_best$Coord),]
    
    #Just in case we have several that have R2:
    
    asatadjbmi_best <- asatadjbmi_best[which.min(as.numeric(asatadjbmi_best$p_value)),]
    ...
    asatadjbmi_best <- asatadjbmi_best[1,]
    
    asatadjbmi_best$lead=snp #we will remove later, just to trace how many we ended up recovering
    
    #Awesome -  let's go and save this:
    
    if(!(exists("recovered_snps"))){
      
      recovered_snps=asatadjbmi_best
      
    } else {
      
      recovered_snps <- rbind(recovered_snps, asatadjbmi_best)
...
non_indels$sample_size = asatadjbmi$sample_size[1] #we can do this since all SNPs have the same sample size for this case
...
lead_df <- asatadjbmi[which(asatadjbmi$chr_pos%in%recovered_snps$chr_pos | asatadjbmi$chr_pos%in%non_indels$chr_pos),] #19 - also removed MHC region!
...
fwrite(lead_df, "output/1_curated_data/asatadjbmi_agrawal_leads.txt")
